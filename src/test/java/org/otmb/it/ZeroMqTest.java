...
public class ZeroMqTest {
    ...
    @Test
    void testPubSub() {
        Message<?> testMessage = MessageBuilder.withPayload("{\"key\": \"value\"}").setHeader("topic", "dnp3/point").build();
    ...
    @Test
    void testPubSub2() {
        String socketAddress = "inproc://vip";
        ZMQ.Socket socket = zContext.createSocket(SocketType.SUB);
        socket.bind(socketAddress);
        socket.subscribe("dnp3/point");

        Message<?> testMessage = MessageBuilder.withPayload("test").setHeader("topic", "dnp3/point").build();
